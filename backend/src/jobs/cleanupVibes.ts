...
import ArchivedVibe from '../models/ArchivedVibe';
...
function extractS3Key(url: string | undefined): string | null {
  if (!url) return null;
...
async function deleteS3Media(vibe: IVibe): Promise<string[]> {
...
async function archiveVibe(vibe: IVibe): Promise<boolean> {
  try {
...
async function cleanupVibes(): Promise<CleanupResults> {
  console.log(`\n${'='.repeat(60)}`);
