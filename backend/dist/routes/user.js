...
const isPlaceholderFirstName = (value) => {
...
const deriveFirstName = (firstName, email) => {
...
router.get('/me', auth_1.authMiddleware, async (req, res) => {
    try {
        const user = await User_1.default.findById(req.userId).select('firstName lastName email profilePicture auraBalance vibeScore betsCreated betsCompleted betsFailed wins losses ducks calloutsReceived calloutsIgnored streak lastActiveDate winRate duckRate contactDiscoveryEnabled');
        if (!user) {
