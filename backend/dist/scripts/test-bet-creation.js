...
async function run() {
    ...
    console.log('\n2. Missing fields → 400');
    try {
        await axios_1.default.post(`${API}/bets/create`, { chatId, betType: 'self' }, // missing description & deadline
        { headers: { Authorization: `Bearer ${token}` } });
        fail('Should reject missing fields');
    }
    catch (err) {
        if (err.response?.status === 400)
            ok('Rejected (400)', err.response.data.error);
        else
            fail('Wrong status', `got ${err.response?.status}`);
    }
    // ── 3. Invalid betType ──────────────────────────────────
    console.log('\n3. Invalid betType → 400');
    try {
        const deadline = new Date();
        deadline.setHours(deadline.getHours() + 6);
        await axios_1.default.post(`${API}/bets/create`, {
            chatId,
...
//# sourceMappingURL=test-bet-creation.js.map