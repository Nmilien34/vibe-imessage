...
async function run() {
    ...
    console.log('─'.repeat(70));
    let user1Token, user1Id, user1BalanceBefore;
    let user2Token, user2Id, user2BalanceBefore;
    let user3Token, user3Id, user3BalanceBefore;
    const chatId = 'test_chat_main';
    try {
        // User 1 (creator)
        const login1 = await axios_1.default.post(`${API}/auth/dev-login`, { userId: 'test_user_me' });
        ...
        const login2 = await axios_1.default.post(`${API}/auth/dev-login`, { userId: 'test_user_friend1' });
        user2Token = login2.data.token;
        user2Id = login2.data.user.id;
        user2BalanceBefore = login2.data.user.auraBalance;
        // User 3 (bettor on NO)
        const login3 = await axios_1.default.post(`${API}/auth/dev-login`, { userId: 'test_user_friend2' });
    ...
    try {
        ...
        const betRes = await axios_1.default.post(`${API}/bets/create`, {
            chatId,
            betType: 'self',
        ...
        await axios_1.default.post(`${API}/bets/${betId}/stake`, { side: 'yes', amount: 60 }, { headers: { Authorization: `Bearer ${user1Token}` } });
        pass('User 1: 60 Aura on YES');
        // User 2 stakes 40 on YES
        await axios_1.default.post(`${API}/bets/${betId}/stake`, { side: 'yes', amount: 40 }, { headers: { Authorization: `Bearer ${user2Token}` } });
        pass('User 2: 40 Aura on YES');
        // User 3 stakes 100 on NO
        await axios_1.default.post(`${API}/bets/${betId}/stake`, { side: 'no', amount: 100 }, { headers: { Authorization: `Bearer ${user3Token}` } });
        ...
        const resolveRes = await axios_1.default.post(`${API}/bets/${betId}/resolve`, { outcome: 'yes' }, { headers: { Authorization: `Bearer ${user1Token}` } });
        pass('Bet resolved successfully');
        // Step 4: Verify payouts
        console.log('\nStep 4: Verifying payouts...');
        // User 1 should get: (60/100) × 200 = 120 (net +60)
        const user1After = await axios_1.default.get(`${API}/user/me`, { headers: { Authorization: `Bearer ${user1Token}` } });
        ...
        const user2After = await axios_1.default.get(`${API}/user/me`, { headers: { Authorization: `Bearer ${user2Token}` } });
        ...
        const user3After = await axios_1.default.get(`${API}/user/me`, { headers: { Authorization: `Bearer ${user3Token}` } });
        ...
        const user1Stats = user1After.data.user;
    ...
    try {
        ...
        try {
            await axios_1.default.post(`${API}/bets/${testBetId}/resolve`, { outcome: 'yes' }, { headers: { Authorization: `Bearer ${user2Token}` } });
    ...
    if (betId) {
        try {
            const res = await axios_1.default.get(`${API}/bets/${betId}/resolution`, { headers: { Authorization: `Bearer ${user1Token}` } });
...
//# sourceMappingURL=test-resolution.js.map