...
async function diagnoseOrphans() {
    try {
        ...
        const usersWithAppleId = await models_1.User.countDocuments({ appleId: { $exists: true, $ne: null } });
        const usersWithoutAppleId = await models_1.User.countDocuments({ appleId: { $exists: false } });
        const usersWithNullAppleId = await models_1.User.countDocuments({ appleId: null });
        ...
        for (const orphan of orphans) {
            if (orphan.firstName) {
                const possibleDuplicate = await models_1.User.findOne({
                    _id: { $ne: orphan._id },
        ...
        for (const orphan of orphans) {
            if (orphan.appleUUID) {
                const match = await models_1.User.findOne({
                    _id: { $ne: orphan._id },
...
diagnoseOrphans();
//# sourceMappingURL=diagnose-orphans.js.map