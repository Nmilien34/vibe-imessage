...
exports.declareObservableOutcome = declareObservableOutcome;
...
const BET_CREATION_FEE_PER_DAY = auraConstants_1.AURA_CONSTANTS.BET_CREATION_FEE_PER_DAY;
const MAX_BET_CREATION_FEE = auraConstants_1.AURA_CONSTANTS.MAX_BET_CREATION_FEE;
const MAX_DESCRIPTION_LENGTH = 500;
const MIN_DEADLINE_HOURS = 1;
const MIN_STAKE = auraConstants_1.AURA_CONSTANTS.MIN_STAKE;
const NEW_STAKE_FEE = Math.max(0, Math.round(auraConstants_1.AURA_CONSTANTS.NEW_STAKE_FEE));
...
function calculateBetCreationCost(params) {
...
async function createBet(input) {
    ...
    if ((creator.auraBalance ?? 0) < totalCost) {
        if (initialStakeFee > 0 || creationCost > 0) {
            throw new Error(`Insufficient Aura. Need ${totalCost} (${creationCost} creation + ${initialStake} stake + ${initialStakeFee} fee), have ${creator.auraBalance ?? 0}`);
        }
...
async function declareObservableOutcome(params) {
    ...
    bet.deadline = new Date(Date.now() + auraConstants_1.AURA_CONSTANTS.OBSERVABLE_DISPUTE_WINDOW_MS);
    await bet.save();
    // Reset votes whenever a fresh declaration starts a new dispute window.
    await ConsensusVote_1.default.deleteMany({ betId });
    return bet;
}
